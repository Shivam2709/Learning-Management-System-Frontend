...
const Profile = () => {
  ...
  const handleClickOutside = (event) => {
    if (isZoomed && !event.target.closest(".zoomable-image")) {
      setIsZoomed(false);
    }
  };
      ...
      <div className="min-h-[90vh] flex items-center justify-center">
        <div className="my-10 flex flex-col gap-4 rounded-lg p-4 text-white w-1/3 shadow-[0_0_10px_black]">
          <img
            ...
            className={`w-40 m-auto cursor-pointer rounded-full border border-black transition-transform duration-300 ${
              isZoomed ? "scale-125" : ""
            } zoomable-image`}
            onClick={handleImageClick}
          />
          <h3 className="text-xl font-semibold text-center capitalize text-purple-500">
            {userData?.fullName}
          </h3>
